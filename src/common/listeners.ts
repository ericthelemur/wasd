...
export function listenTo<T extends keyof LTs>(name: T, listener: Listener<LTs[T]['type']>) {
...
export function sendToF<T extends keyof LTs>(name: T, data: LTs[T]['type']) {
    return () => {
...
export function sendTo<T extends keyof LTs>(name: T, data: LTs[T]['type']) {
    sendToF(name, data)();
}
