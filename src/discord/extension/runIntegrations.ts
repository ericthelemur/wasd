...
sc.runDataActiveRun.on("change", async (newVal) => {
    ...
    if (!runStatus.messageID) {     // If no message yet sent, send message with random format
        ...
        const channel = discord.client?.channels.cache.get(login.scheduleChannel) as TextChannel;
        sendTo("previewMessage", { runID: newVal.id, runName: newVal.game || "", channelID: login.scheduleChannel, channelName: channel?.name, content: message });
    }
})
...
listenTo("postMessage", async (data, ack) => {
    ...
    else sendError(ack, `Error sending message to ${data.channelID}`);
})
